...
abstract contract Staking721TokenId is ReentrancyGuard, IStaking721 {
    /*///////////////////////////////////////////////////////////////
    ...
    uint256[] public stakingTokenAllowedId;
    ...
    constructor(address _stakingToken, uint256[] memory _stakingTokenAllowedId) ReentrancyGuard() {
        require(address(_stakingToken) != address(0), "collection address 0");
        require(uint256(_stakingTokenAllowedId.length) > 0, "No allowed token id");
        stakingTokenAllowedId = _stakingTokenAllowedId;
...
function _stake(uint256[] calldata _tokenIds) internal virtual {
    ...
    require(len < 1001, "Maximum 1000 tokens at once");

    bool isAllowed = false;
    uint256 a = stakingTokenAllowedId.length;
    for (uint256 n = 0; n < len; ++n) {
        isAllowed = false;
        for(uint256 i = 0; i < a; ++i){
            if (stakingTokenAllowedId[i] == _tokenIds[n]) {
                isAllowed = true;
                break;
            }
        }
        if(isAllowed == false){
        revert("Not authorized tokens ID");
        }
    }
